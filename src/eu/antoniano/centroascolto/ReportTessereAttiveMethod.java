...
public class ReportTessereAttiveMethod extends GiveMethod {
	...
	public ReportTessereAttiveMethod(DefaultModule parentModule, String[] defaultParameters) {
	...
	protected Document retrieveData(HttpServletRequest request, ReportEngine engine, File designFile)
		...
		try {
			...
			currentElement.setText(calendario);
			...
			for (TesseraDTO t : tessere.values()) {
				if ((calendario.toLowerCase().contains("italia") && t.italiano) || (!calendario.toLowerCase().contains("italia") && !t.italiano))	{
